...
提供：
- POST /api/reviews             创建并同步执行 Review（不引入任务队列）
- GET  /api/reviews/{review_id} 查询 Review 结果（review_id 即 project_id）

结果持久化在 uploads/{project_id}/review.json，重复审查会覆盖旧结果；
响应携带完整运行报告（status/counts/coverage/unresolved/errors）与项目级
Fix Prompt，前端无需二次请求。
"""
import json
import re
...
from app.models.schemas import ReviewRequest, ReviewResponse, RunReport
...
def create_review(request: ReviewRequest) -> ReviewResponse:
    """同步执行完整审查流程：分析 -> 发现-核实管线 -> 生成 Fix Prompt。"""
...
def get_review(review_id: str) -> dict:
    """查询已保存的 Review 结果（含完整运行报告与项目级 Fix Prompt）。"""
